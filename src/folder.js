...
class Folder extends React.Component {
  ...
  checkPathname = async (pathname) => {
    ...
    const user = (isProfile, id) => {
      if (!this.props.profile || this.props.profile.username !== id) {
        console.log(isProfile.username + " profile");
        if (isProfile) {
          this.props.loadGreenBlue("getting entities by " + isProfile.username);
          isProfile && this.props.getProfileEntities(isProfile);
        }
      } else {
      ...
      <Foundation
        ...
        allowDeviceToRead={() => {
          if (user !== undefined && auth !== undefined) {
            fumbleThruKeys(auth, user, rsaPrivateKeys)
              .then((obj) => {
