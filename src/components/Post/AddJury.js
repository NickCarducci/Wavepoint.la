...
class AddJury extends React.Component {
  state = {
    userQuery: ""
  };
  render() {
    return (
      ...
      >
        ...
        >
          <div
            style={{
              color: "none",
              position: "absolute",
              transform: "rotate(180deg) translate(-55px,3px)",
          ...
          {this.state.showJury ? "Requests" : "Jury"}
          &nbsp;&nbsp;&nbsp;&nbsp;
        ...
        <input
          ...
          placeholder="New jury"
        />
        <div
          style={
            this.props.x.jury && this.props.x.jury.length > 4
        ...
        >
          ...
          >
                ...
                if (this.props.x && x.username.includes(this.state.userQuery)) {
                  return (
                    <div
                      ...
                      onClick={() => {
                        if (this.props.x.jury.includes(x.id)) {
                          var answer1 = window.confirm(
                            `remove ${x.name}@${x.username} as jury?`
                          );
                          ...
                          if (answer1)
                            firebase
                              ...
                              .doc(this.props.x.id)
                              .update({
                                jury: firebase.firestore.FieldValue.arrayRemove(
                                  x.id
                                )
                              })
                          ...
                          if (answer)
                            firebase
                              ...
                              .doc(this.props.x.id)
                              .update({
                                jury: firebase.firestore.FieldValue.arrayUnion(
                                  x.id
                                )
                              })
                              .catch((err) => console.log(err.message));
                    ...
                    >
                      ...
                      />
                      {x.name}@{x.username}
                      {this.props.x.jury && this.props.x.jury.includes(x.id) ? (
                        <div
                          style={{
                            marginLeft: "4px",
                            color: "grey",
            ...
            {this.state.userQuery === "" && this.state.showJury
              ? this.props.x &&
                this.props.x.requestingJury.map((x) => {
                    ...
                    >
                      ...
                      <div>
                        <div
                          onClick={() => {
                            var answer = window.confirm(
                              `accept ${x.name}@${x.username} as jury?`
                            );
                        ...
                        <div
                          onClick={() => {
                            var answer = window.confirm(
                              `reject ${x.name}@${x.username} as jury?`
                            );
