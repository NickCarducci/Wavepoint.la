...
class AddConsult extends React.Component {
  state = {
    userQuery: ""
  };
  render() {
    return (
      ...
      >
        <div
          onClick={
            this.state.showConsultancy
              ? () => this.setState({ showConsultancy: false })
              : () => this.setState({ showConsultancy: true })
          }
        ...
        >
          ...
          {this.state.showConsultancy ? "Requests" : "Testimonies"}
          &nbsp;&nbsp;&nbsp;&nbsp;
        ...
        <div
          style={
            this.props.x.consults && this.props.x.consults.length > 4
        ...
        >
          ...
          >
                ...
                if (this.props.x && x.username.includes(this.state.userQuery)) {
                  return (
                    <div
                      ...
                      onClick={() => {
                        if (this.props.x.consults.includes(x.id)) {
                          if (
                            (this.props.x.prosecution &&
                            ...
                            if (answer1)
                              firebase
                                ...
                                .doc(this.props.x.id)
                                .update({
                                  consults: firebase.firestore.FieldValue.arrayRemove(
                                    x.id
                                  )
                                })
                          ...
                          if (answer)
                            firebase
                              ...
                              .doc(this.props.x.id)
                              .update({
                                consults: firebase.firestore.FieldValue.arrayUnion(
                                  x.id
                                )
                              })
                              .catch((err) => console.log(err.message));
                    ...
                    >
                      ...
                      />
                      {x.name}@{x.username}
                      {this.props.x.consults &&
                      this.props.x.consults.includes(x.id) ? (
                        <div
                          style={{
                            marginLeft: "4px",
                            color: "grey",
            ...
            {this.state.userQuery === "" && this.state.showConsultancy
              ? this.props.x &&
                this.props.x.requestingConsultancy.map((x) => {
                    ...
                    >
                      ...
                      <div>
                        <div
                          onClick={() => {
                            var answer = window.confirm(
                              `accept ${x.name}@${x.username} as able to give a consultancy?`
                            );
                        ...
                        <div
                          onClick={() => {
                            var answer = window.confirm(
                              `reject ${x.name}@${x.username} as able to give a consultancy?`
                            );
