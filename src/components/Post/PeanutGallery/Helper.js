...
import VideoRecorder from "../../../widgets/Video/VideoRecorder.js";

class Helper extends React.Component {
  state = { fire: false };
  render() {
    const { confirmDelete } = this.state;
    const { parent, openingThisOne } = this.props;
    const lit =
              ...
              onSubmit={(e) => {
                ...
                const deleteComments = () => {
                  const comments = firebase
                    .firestore()
                    .collection(parent.commentsName);
                  comments
                    ...
                    <img
                      ...
                      onClick={() =>
                        this.setState(
                          {
                            reaction:
                              reactions[reactions.lastIndexOf(reaction) + 1]
                          },
                          (reaction) => {
                            const newReaction =
                              reactions[reactions.lastIndexOf(reaction) + 1];
                  ...
                  if (!confirmDelete) {
                    var answer = window.confirm(
                      `want to really delete ${
                        ...
                        parent.message
                      } and all comments therein? you cannot undo this!`
                    );
                    if (answer) {
                      this.setState({
                        confirmDelete: ""
          ...
          {!this.props.isDroppedIn &&
            ...
            !this.state.videoRecorderOpen && (
              <div
                ...
                onClick={() => this.setState({ videoRecorderOpen: true })}
              >
                +
              </div>
            )}
