...
class Chatter extends React.Component {
  ...
  handleKeysForRoom = (room) => {
    const { recipientsProfiled, threadId, keyBoxes, user } = this.props;
          ...
          rsa
            ...
            .then(async (privateRoomKey) => {
              if (keyBox.key !== privateRoomKey) {
                ...
                keyBoxConfirmed &&
                  console.log("keyBox established locally for " + threadId);
              } else
                console.log(
                  "there is a keyBox already registered for " + threadId
                );
            });
      } else {
    ...
    if (room.publicRoomKey) {
      ...
      rsa.generateKey(2048).then(async (roomBox) => {
        if (
          ...
          Promise.all(
            ...
            saltedKeys.map((out, p) => {
              ...
              return (rm["saltedKeys" + user.id] =
                user && out.substring(0, out.lastIndexOf(saltedKeys)));
            });
            var rm = { ...room };
            delete rm.id;
            rm.box = roomBox.box;
            if (p === saltedKeys.length)
              firebase.firestore().collection("rooms").doc(room.id).update(rm);
          });
        }
      });
    }
