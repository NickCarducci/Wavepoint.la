...
import EventTypesMap from "./components/Map/EventTypesMap";
...
class Foundation extends React.Component {
  ...
  newforum = () => {
    const { forumOpen, community, auth } = this.props;
    if (forumOpen || canIView(auth, null, community)) {
      this.setState({ openNewForum: true });
    } else if (this.state.createSliderOpen) {
      this.createSliderCloser();
    } else {
      this.createSliderOpener();
      this.eventCloser();
    }
  };
  queryMount = (pathname) => {
  ...
  chooseCitypoint = async (
    ...
    this.setState(
      ...
      () => {
        ...
        if (tileChosen === "department" || tileChosen === "classes") {
          if (
    ...
    if (community.center) {
      this.setState(
        ...
        () => {
          ...
          if (
            ["forum", "ordinances", "department", "forms & permits"].includes(
              this.props.commtype
            ) &&
    ...
    this.setState(
      {
        showFilters: true,
        started: false
        //subForum: !["classes", "department"].includes(this.props.commtype)
      },
      () =>
        this.props.setIndex({
          forumOpen: true
  ...
  render() {
    ...
    var forumPosts = ["new", "lesson", "show", "game"].includes(
      ...
      ? this.props.classes
      : this.props.commtype === "department"
