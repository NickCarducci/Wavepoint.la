          ...
          <div>
            ...
            >
              <Post
                ...
                city={drop.city}
                commtype={
                  ["budget & proposals", "oldBudget"].includes(drop.collection)
                    ? "budget & proposal"
                    : ["elections", "oldElections"].includes(drop.collection)
                    ? "election"
                    : ["court cases", "oldCases"].includes(drop.collection)
                    ? "court case"
                ...
                helper={(x) => {
                  var input = x ? x : null;
                  this.props.helper(input, 1);
                  this.setState(
                    {
                      chosenPostId: drop.id,
                      postHeight: this.only.current.offsetHeight
                    },
                    () => {
                      this.only.current.scrollIntoView("smooth");
                    }
                  );
                }}
            ...
            <PeanutGallery
              ...
              helper={(x) => {
                ...
                if (!this.props.comments) {
                  window.alert("that's weird :/ helper");
                } else {
                  this.setState({
                    postHeight: 0
                  });
